...
let wsClient = $derived(gameId != '' ? new WebSocket(`ws://${PUBLIC_API_URL}/ws/${gameId}`) : undefined);
...
export async function createGame(): Promise<string> {
    // post to the server
    let res = await fetch("http://"+PUBLIC_API_URL+"/create-game", {method: "POST"});
...
export async function joinGame(_gameId: string) {
    ...
    let clientId = localStorage.getItem(`clientId-${gameId}`) || generateClientId();
